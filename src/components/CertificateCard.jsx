...
const CertificateCard = ({ title, academy, exp, link, image, release }) => {
  const { locale } = useContext(themeContext);
  return (
    <motion.div initial={{ translateY: 75, opacity: 0 }} whileInView={{ translateY: 0, opacity: 1 }} transition={{ duration: 0.6, ease: ['easeInOut'] }} className={`shadow rounded-md p-2 ${ locale === 'light' ? 'bg-gray-100 shadow-xl':'gradient-primary' }`}>
