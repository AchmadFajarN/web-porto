...
const CertificateCard = ({ title, academy, time, link, image }) => {
  return (
    <motion.div initial={{ translateY: 300, opacity: 0 }} whileInView={{ translateY: 0, opacity: 1 }} transition={{ duration: 0.6, ease: ['easeInOut'] }} className="gradient-primary shadow rounded-md p-2">
      <div style={{ backgroundImage: `url(${ image })` }} className="rounded-md bg-cover bg-center w-full h-40"></div>
      <div className="mt-4">
        <h2 className="font-semibold">{title}</h2>
        <p className="text-sm text-gray-50/50">{academy}</p>
